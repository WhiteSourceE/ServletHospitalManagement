 ...
 */
@WebServlet("/DischargeSafe")
public class DischargeSafe extends HttpServlet {
    private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public DischargeSafe() {
        super();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String sq1 = "delete from patient where pid = "+escapeSql(pid);
        try {
            ...
            for(String mc:mcs){
                ...
                String sql_name = "select name from medicine where mid = "+escapeSql(mid);
                ResultSet rs_name = c.createStatement().executeQuery(sql_name);
                rs_name.next();
                String name = rs_name.getString("name");
                sb.append( name + " ");

                String sql_price = "select price from medicine where name = "+escapeSql(name);
