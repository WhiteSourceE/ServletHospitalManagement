...
@WebServlet("/AddPatientJavascriptAttribute")
public class AddPatientJavascriptAttributeResource extends HttpServlet {
    ...
    private void unsafe(HttpServletRequest request, HttpServletResponse response) {
        ...
        savedPatientDto.ifPresent(thePatient -> {
            try {
                ...
                out.println("<a onclick=\"alert('" + thePatient.getName() + "');\">");
    ...
    private void safe(HttpServletRequest request, HttpServletResponse response) {
        ...
        savedPatientDto.ifPresent(thePatient -> {
            try {
                ...
                PrintWriter out = response.getWriter();
                out.println("<br><br><h1 align=center><font color=\"red\">Unsafe Javascript Attribute context:<br></font></h1>");
                out.println("<a onclick=\"alert('" + Encode.forJavaScriptAttribute(thePatient.getName()) + "');\">");
                out.println("Click me</a>");
                out.println("</script>");
            } catch (IOException e) {
                e.printStackTrace();
