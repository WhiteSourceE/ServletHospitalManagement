...
@WebServlet("/AddPatientStyleAttributeCssString")
public class AddPatientStyleAttributeCssStringResource extends HttpServlet {
    ...
    private void unsafe(HttpServletRequest request, HttpServletResponse response) {
        ...
        savedPatientDto.ifPresent(thePatient -> {
            try {
                ...
                out.write("<div style=\"color:" + thePatient.getName() + "\">");
    ...
    private void safe(HttpServletRequest request, HttpServletResponse response) {
        ...
        savedPatientDto.ifPresent(thePatient -> {
            try {
                response.setContentType("text/html");
                PrintWriter out = response.getWriter();
                out.write("<br><br>Safe CSS style attribute context:<br>");
                out.write("<div style=\"color:" + Encode.forCssString(thePatient.getName()) + "\">");
                out.write("<h1>TEXT</h1>");
                out.write("</div>");
            } catch (IOException e) {
                e.printStackTrace();
