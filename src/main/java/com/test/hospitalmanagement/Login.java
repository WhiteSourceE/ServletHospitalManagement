		...
		try {
			...
			if(rs.next() && rs.getString("password").equals(pwd)){
				...
				if(Util.isValidEmailAddress(email)) {
					out.println("<h1 align=center>Email Validated</h1>");
					out.println("<h3 align=center>" +  email + "</h3>");
				}
				out.println("<br><br><br><h3 align=center>Page will be redirected 5 seconds</h3>");
				out.println("<script type=\"text/javascript\">redirectURL = \"welcome.html\";setTimeout(\"location.href = redirectURL;\",\"5000\");</script>");
