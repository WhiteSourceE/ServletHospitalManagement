 ...
 */
@WebServlet("/RetrievePatientsDIDSafe")
public class RetrievePatientsDIDSafe extends HttpServlet {
    private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public RetrievePatientsDIDSafe() {
        super();
...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
        ...
        String sql = "select patients,name from doctor where did = "+ ESAPI.encoder().encodeForSQL(codec, did);
        try {
            ...
            for(String p: pList) {
                if(Integer.valueOf(p) >= 0){
                    ...
                    ResultSet rr = ss.executeQuery("select * from patient where pid = "+ESAPI.encoder().encodeForSQL(codec, p));
                    rr.next();
                    out.println("<tr>");
